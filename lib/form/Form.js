...
class Form {
  ...
  dsp = async () => {
    ...
    html += `<a class="btn btn-danger me-2" title="${lang.BTN_CANCEL}" href="${this.cancelUrl}">${lang.BTN_CANCEL}</a>`;
    html += `<button class="btn btn-primary" title="${lang.BTN_SAVE}">${lang.BTN_SAVE}</button>`;
    html += `</div>`;
    // close form tag
    html += `</form>`;
    return html;
